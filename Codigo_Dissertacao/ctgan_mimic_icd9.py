...
def pres():
    ...
    with open("../Output/mimic_output_prescriptions_ctgan.csv", "w") as my_empty_csv:
    ...
    for n in range(10):
        ...
        synth_df.to_csv(
            '../Output/mimic_output_prescriptions_ctgan.csv', mode='a', index=False)
        gc.collect()
...
def proc():
    ...
    print(synth_df.head())
    #new_data = new_data.append(synth_df)
    synth_df.to_csv('../Output/mimic_output_procedures_ctgan.csv', index=False)
    gc.collect()
...
def diag():
    ...
    for n in range(2):
        ...
        #new_data = new_data.append(synth_df)
        synth_df.to_csv(
            '../Output/mimic_output_diagnoses_ctgan.csv', mode='a', index=False)
        gc.collect()
